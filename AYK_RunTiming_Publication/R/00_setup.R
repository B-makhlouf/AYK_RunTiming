...
# File paths are centralised in config.R (the single source of truth).
# Edit config.R, NOT this file, to point at data/outputs on your system.
if (!exists("PROJECT_ROOT")) source("config.R")

PATHS <- list(
  # Spatial data (place shapefiles in data/spatial/ ; see config.R / README)
  kusko_edges = KUSKO_EDGES,
  kusko_basin = KUSKO_BASIN,
  yukon_edges = file.path(SPATIAL_DIR, "YukonUSGS.shp"),          # unused (Kuskokwim-only paper)
  yukon_basin = file.path(SPATIAL_DIR, "Yuk_Mrg_final_alb.shp"),  # unused (Kuskokwim-only paper)

  # Natal-origin genetics + CPUE data (shipped in data/natal_origins/)
  natal_data_dir = NATAL_DATA_DIR,
...
cat("✓ Paths are configured in config.R (edit that file for your system).\n")
cat("✓ Remember to load R/01_plot_functions.R for plotting functions.\n")