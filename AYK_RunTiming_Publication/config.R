...
#
# PROJECT_ROOT is detected automatically (via the 'here' package if installed,
# anchored by AYK_RunTiming_Publication.Rproj; otherwise the working directory).
#
# WHAT YOU MUST SUPPLY: the two Kuskokwim shapefiles (with sidecar files) under
#   data/spatial/Spatial Data/KuskoUSGS_HUC_joined.shp
#   data/spatial/Spatial Data/Kusko_basin.shp
# (French et al. 2020 strontium isoscape; see README.)
################################################################################
...
CLUSTER_GROUPS_DIR   <- file.path(OUTPUT_ROOT, "Management_Distinguishability")   # clustering figs
CLUSTER_ANALYSIS_DIR <- file.path(OUTPUT_ROOT, "Cluster Analysis Results")        # Figs 3-6

# ---- FINAL FIGURE EXPORTS (paper-ordered deliverables) ----------------------
FIGURES_EXPORT_DIR     <- file.path(PROJECT_ROOT, "Figures")
CLUSTERING_FIGURES_DIR <- file.path(FIGURES_EXPORT_DIR, "Clustering")
...
# ---- SHARED ANALYSIS PARAMETERS (unchanged from the paper) ------------------
CFG <- list(
  years                    = c(2017, 2018, 2019, 2020, 2021, 2022),
  watersheds               = c("Kusko"),
  cumulative_interval_days = 3,
  closure_start_doy        = 153,   # June 1  (used by R/06)
  closure_end_doy          = 162,   # June 11 (used by R/06)
