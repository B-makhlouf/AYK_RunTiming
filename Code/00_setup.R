...
################################################################################
# OPTIONAL: CLUSTER MAPPING
################################################################################

# Source cluster mapping if available
cluster_script_path <- file.path(dirname(rstudioapi::getActiveDocumentContext()$path), "00b_cluster_mapping.R")
if (file.exists(cluster_script_path)) {
  source(cluster_script_path)
} else {
  # Try relative path
  if (file.exists("Code/00b_cluster_mapping.R")) {
    source("Code/00b_cluster_mapping.R")
  }
}