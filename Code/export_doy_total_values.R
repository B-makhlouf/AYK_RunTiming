# export_doy_total_values.R
# Function to export DOY total analysis results to CSV files
...
#' Export DOY total analysis results to CSV files
#'
...
#' @param output_dir Directory to save output CSV files
#' @return List with paths to created CSV files
export_doy_total_analysis_results <- function(years, watersheds, 
  ...
  extract_trib_data <- function(trib_data, percent_of_total, metadata) {
    # Early return if input is NULL
    if (is.null(trib_data)) return(NULL)
    
    # Add percent_of_total_run if provided
    if (!is.null(percent_of_total)) {
      trib_data$percent_of_total_run <- percent_of_total
    }
    ...
    if (inherits(trib_data, "sf")) {
      ...
      result <- data.frame(
        ...
        percent_of_total_run = trib_data$percent_of_total_run
      )
  ...
  for (watershed in watersheds) {
    for (year in years) {
      message(paste("Processing DOY total analysis results for", watershed, "watershed, year", year))
      ...
      # Process DOY quartile data with total normalization
      message("  Processing DOY quartile data with total normalization")
      
      tryCatch({
        doy_results <- DOY_Total_Analysis(
          year = year,
          watershed = watershed,
        ...
        for (q in 1:length(doy_results)) {
          ...
          doy_trib_df <- extract_trib_data(
            edges, 
            doy_results[[q]]$percent_of_total_run, 
            doy_metadata
          )
      ...
      # Process cumulative DOY data with total normalization if requested
      if (include_cumulative) {
        message("  Processing cumulative DOY data with total normalization")
        
        tryCatch({
          cum_doy_results <- Cumulative_DOY_Total_Analysis(
            year = year,
            watershed = watershed,
            sensitivity_threshold = sensitivity_threshold,
            min_error = min_error,
          ...
          for (q in 1:length(cum_doy_results)) {
            ...
            cum_doy_metadata <- list(
              year = year,
              watershed = watershed,
              analysis_type = "Cumulative_DOY_Total",
              quartile = paste0("Q", q)
            )
            ...
            cum_doy_trib_df <- extract_trib_data(
              edges, 
              cum_doy_results[[q]]$percent_of_total_run, 
              cum_doy_metadata
            )
  ...
  if (is.null(all_huc_data) || nrow(all_huc_data) == 0) {
    ...
    huc_filepath <- file.path(output_dir, "all_doy_total_huc_values.csv")
  ...
  if (is.null(all_trib_data) || nrow(all_trib_data) == 0) {
    ...
    trib_filepath <- file.path(output_dir, "all_doy_total_tributary_values.csv")
