...
DOY_Total_Analysis <- function(year, watershed, sensitivity_threshold, min_error, 
  ...
  # First, calculate the TOTAL production for the full run (all quartiles combined)
  ...
  # Calculate total HUC percentages across the entire run
  # This is important for the histogram comparison
  ...
  for (q in 1:length(quartile_subsets)) {
    ...
    message(paste("Processing", subset_labels[q], "with", nrow(current_subset), "data points (as % of total run)"))
    ...
    # Calculate DIRECT proportion of total run production (not normalized within quartile)
    ...
    # Process HUC data using the direct percentage values
    ...
    main_plot <- ggplot() +
      ...
      coord_sf(datum = NA) +
      labs(
        title = paste0(subset_labels[q], ": Percent of Total Run - ", watershed, " Watershed"),
    ...
    # Create the updated histogram showing total vs. quartile contribution
    huc_histogram <- create_huc_total_quartile_histogram(
      final_result = final_result,
      all_hucs_data = all_hucs_result,
      quartile_label = subset_labels[q]
    )
    ...
    # If we have a DOY histogram to include, add it at the bottom spanning both columns
    if (!is.null(gg_hist)) {
    ...
    if (return_values) {
      all_results[[q]] <- list(
        ...
        all_hucs_result = all_hucs_result  # Include the total results for reference
      )
    }
  }
